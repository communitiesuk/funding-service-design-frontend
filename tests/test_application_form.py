...
from app.config import LOCAL_FORMS_SERVICE_ROOT
...
class TestFormURLsWithChrome:
    ...
    def get_form_service_host(form_dir):
        if FORMS_SERVICE_NAME == LOCAL_SERVICE_NAME:
            return (
                url_for("routes.index", _external=True)
                + LOCAL_FORMS_SERVICE_ROOT
            )
        else:
    ...
    def test_form_pages_accessible(self):
        ...
        for form_dir, form_name, pages in self.get_test_form_pages():
            for page in pages:
                ...
                if FORMS_SERVICE_NAME == LOCAL_SERVICE_NAME:
                    route_rel = (
                        "/" + LOCAL_FORMS_SERVICE_ROOT + "/" + route_rel
                    )
    ...
    def test_insecure_form_post_returns_csrf_forbidden(self):
        """
        GIVEN Our FORM SERVICE Application is running
        ...
        for form_dir, form_name, pages in self.get_test_form_pages():
            for page in pages:
                ...
                response = self.client.post(
                    route_rel, data={"name": "Bad Data"}
                )
                assert response.status_code == 400, response
                assert b"CSRF token is missing" in response.data, response.data
